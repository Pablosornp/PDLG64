...
class AnalizadorSinSem:
    ...
    def P(self):
        ...
        if (self.sigToken in first1):
            self.fichParse.write(' 1')
            self.B()
            self.P()
        elif (self.sigToken in first2):
            self.fichParse.write(' 2')
            self.F()
            self.P()
        elif (self.sigToken in first3):
    ...
    def C(self):
        first1 = [("PR",6),("ID",None),("PR",4),("PR",12),("PR",13),("PR",7),("PR",10),"var",("PR",14)] #First(B C)={ break id if print prompt return switch var while }
    ...
    def equiparaToken(self,tok):
        if tok[0] == self.sigToken[0] and tok[1] == self.sigToken[1]:
